import { Component, OnInit, ChangeDetectorRef, inject } from '@angular/core'; // <-- 1. Importamos el "Martillo"
...
export class DashboardDocenteComponent implements OnInit {
  ...
  cargarMisGrupos(): void {
    this.materiaService.obtenerMaterias().subscribe((todasLasMaterias) => {
      this.misMaterias = todasLasMaterias.filter(
        (m: any) => m.docente && m.docente.id === this.usuarioActual.id,
      );
    });
  }
    ...
    this.inscripcionService.obtenerPorMateria(materia.id).subscribe({
      next: (datos) => {
        if (datos && datos.length > 0) {
          ...
          this.alumnosInscritos = datos
            .map((ins: any) => ins.estudiante)
            .filter((e: any) => e != null);
        }
  ...
  obtenerTotalHoras(): number {
    return this.misMaterias.reduce((total, m) => total + m.frecuenciaSemanal * 2, 0);
  }
  obtenerTotalEstudiantes(): number {
    return this.misMaterias.reduce((total, m) => total + m.cupos, 0);
  }
